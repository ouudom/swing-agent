# auto-swing — Thin Application Migration Plan
...
> - **One compact repo `auto-swing/`, exactly two folders: `wiki/` + `src/`.** `wiki/` is the AI's
...
Everything is one repo, `auto-swing/`, with exactly two folders. `wiki/` is the AI's execution
...
```
auto-swing/                         ← ONE repo · `live` branch = command + routine read/write
  wiki/                             ← AI EXECUTION CONTEXT ONLY (curated, no parent-wiki copy)
    ...
    weekly-forecasts/{instrument}/YYYY-WNN.md ← /weekly output; AI reads back as context
    validations/{instrument}/YYYY-MM-DD.md    ← /validate output; AI reads back as context
...
| **Weekly forecast prose** — `wiki/weekly-forecasts/{instrument}/*` | **repo, `live` branch** | Claude Code `/weekly` writes (native `Write`) + `git commit`; reads its own prior output back as context | AI-owned output trail. Starts from new auto-swing runs, not copied history. Structured zones go to DB via `publish_zone`. |
| **Validation prose** — `wiki/validations/{instrument}/*` | **repo, `live` branch** | Claude Code `/validate` writes (native `Write`) + `git commit`; reads its own prior output back as context | AI-owned output trail. Starts from new auto-swing runs, not copied history. Structured verdicts go to DB via `write_verdict`. |
...
- `system/{instrument}/{confluence_criteria,*_profile}.md`
- `templates/{weekly_forecast,daily_validation}.md`
- `weekly-forecasts/{instrument}/YYYY-WNN.md` written by auto-swing `/weekly`
- `validations/{instrument}/YYYY-MM-DD.md` written by auto-swing `/validate`

Excluded from `auto-swing/wiki/`:
...
- `research/*` — analysis history stays outside the deployed auto-swing repo.
- historical `forecasts/*` / `daily/*` from the old base repo — do not bulk-import them. The new
  `weekly-forecasts/` and `validations/` folders are populated only by auto-swing runs.
...
- **Weekly** (`/weekly`, Mon): read the pair's rules + last auto-swing forecast from `wiki/` → `get_brief` +
  `sql_query`/`run_backtest` as needed → apply zone-selection + R1 judgment → `git`-write
  `wiki/weekly-forecasts/{inst}/{week}.md` → `publish_zone` for each published zone.
- **Hourly validate** (`/validate`): read rules + this week's forecast from `wiki/` → `get_brief`
  ...
  the bias-flip / re-forecast call → `git`-write `wiki/validations/{inst}/{date}.md` →
  `write_verdict`.
- **Ad-hoc context / research** (you ask): `get_brief` / `sql_query` / `run_backtest` / `run_replay`
  → analysis returned to you. auto-swing stays compact (no `research/` folder); persist any writeup
  outside the deployed auto-swing repo if you want it kept. This is the "deep review / backtest offset+EC"
...
|---|---|---|
| `/weekly` (manual, laptop) | `wiki/weekly-forecasts/{inst}/*`, `wiki/system/core/macro/yield_environment.md` | `wiki/*` rules, last auto-swing forecast |
| `/validate` (hourly routine) | `wiki/validations/{inst}/*` — **nothing else** | `wiki/*` rules, this week's weekly forecast, `get_brief` |

Weekly and daily forecast paths never overlap (different dirs, different filenames). `yield_environment`
...
… read curated wiki context + prior auto-swing output, do judgment …
...
**Goal:** create the compact `auto-swing/` shape, then change storage only. Deployment root is
`auto-swing/`; deterministic engine scripts are vendored under `src/engine/scripts/`.

**0.0 Curated `wiki/` skeleton — DONE 2026-07-04**
- Create only `auto-swing/wiki/` + `auto-swing/src/`.
...
- Seed rules by curated extraction only: active execution rules into `system/*`, templates into
  `templates/*`; first auto-swing run creates the first weekly forecast / validation output.
...
- Add pipeline Docker service; compose bind-mounts the `auto-swing/` root only (`/app == auto-swing`).
...
- Validate owns only `wiki/validations/{inst}/*`.
...
- Weekly owns only `wiki/weekly-forecasts/{inst}/*` and `yield_environment.md`.
