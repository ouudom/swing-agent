# auto-swing Ubuntu Deployment Guide

Target: Ubuntu server running Docker Compose from the `auto-swing/` repo/folder only. Do **not**
...
mkdir -p ~/apps
cd ~/apps
git clone <AUTO_SWING_REPO_URL> auto-swing
cd auto-swing
```

Use the branch carrying the standalone `auto-swing` folder as repo root.
...
```cron
0 23 * * 1-5 cd ~/apps/auto-swing/src && ./scripts/backup_postgres.sh >> backups/backup.log 2>&1
```
